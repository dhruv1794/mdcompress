...
const (
	AnthropicBackendName     = "anthropic"
...
func (b *AnthropicBackend) Name() string {
	return AnthropicBackendName
}

func (b *AnthropicBackend) Complete(prompt string) (string, error) {
	if strings.TrimSpace(b.Model) == "" {
		return "", fmt.Errorf("anthropic eval backend requires a model")
	}
	apiKey := strings.TrimSpace(os.Getenv(b.APIKeyEnv))
	if apiKey == "" {
		return "", fmt.Errorf("anthropic eval backend requires %s", b.APIKeyEnv)
	}
	client := b.Client
	if client == nil {
		client = &http.Client{Timeout: 2 * time.Minute}

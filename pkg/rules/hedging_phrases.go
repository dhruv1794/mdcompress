...
var hedgingReplacements = []hedgingReplacement{
	...
	{regexp.MustCompile(`(?i)\blet['']s (?:take a look at|have a look at|dive into|explore|examine)\s+`), ""},
	{regexp.MustCompile(`(?i)\bas you can see,?\s+`), ""},
	{regexp.MustCompile(`(?i)\bnow,?\s+what['']s (really |truly )?interesting is\s+`), ""},
	{regexp.MustCompile(`(?i)\bit is important to understand that\s+`), ""},
	{regexp.MustCompile(`(?i)\bfirst of all,?\s+`), ""},
	{regexp.MustCompile(`(?i)\bat the end of the day,?\s+`), ""},
	{regexp.MustCompile(`(?i)\bthe bottom line is,?\s+`), ""},
	{regexp.MustCompile(`(?i)\byou might be wondering,?\s+`), ""},
	{regexp.MustCompile(`(?i)\binterestingly enough,?\s+`), ""},
	{regexp.MustCompile(`(?i)\bin this (?:section|chapter|guide|tutorial),? we['']ll?\s+`), "we'll "},
	{regexp.MustCompile(`(?i)\bkeep (?:it )?in mind that\s+`), ""},
	{regexp.MustCompile(`(?i)\bbear in mind that\s+`), ""},
	{regexp.MustCompile(`(?i)\bmake sure to\s+`), ""},
	{regexp.MustCompile(`(?i)\bdo not forget to\s+`), ""},
	{regexp.MustCompile(`(?i)\bit (?:is |goes without saying that it is )(?:also )?(?:worth |interesting to )?(?:noting|mentioning|pointing out) that\s+`), ""},
	{regexp.MustCompile(`(?i)\bas a general rule of thumb,?\s+`), ""},
	{regexp.MustCompile(`(?i)\bwithout further ado,?\s+`), ""},
	{regexp.MustCompile(`(?i)\bto (?:put|make a long story) (?:it |things )?(?:simply|short),?\s+`), ""},
	{regexp.MustCompile(`(?i)\bfor what it['']s worth,?\s+`), ""},
	{regexp.MustCompile(`(?i)\b(?:this |that )?being said,?\s+`), ""},
	{regexp.MustCompile(`(?i)\bthat said,?\s+`), ""},
	{regexp.MustCompile(`(?i)\bif you will,?\s+`), ""},
